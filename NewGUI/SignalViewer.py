...
class Viewer(QWidget):
    ...
    def ViewerUi(self):
        ...
        self.canvas = MplCanvas(self, width=5, height=4, dpi=100)
        self.plotting_instance = Plotting(self.canvas, self.timeLabel)
        if self.data_list:
            self.plotting_instance.init_plot(self.data_list)
    ...
    def setup_connections(self):
        ...
        self.play_signal.connect(self.plotting_instance.play)
        self.pause_signal.connect(self.plotting_instance.pause)
        self.rewind_signal.connect(self.sync_with)
        self.plotting_instance.rewind_state_changed.connect(self.update_rewind_button)

    def sync_with(self, other_viewer):
        """Sync current time and state with another viewer."""
        if other_viewer:  # Check if other_viewer is defined
            self.current_time = other_viewer.current_time
            self.update_plot()  # Update the plot based on the new time
            # Sync rewind state
            if hasattr(other_viewer, 'rewindButton'):
                self.rewindButton.setChecked(other_viewer.rewindButton.isChecked())
                self.plotting_instance.rewind_state_changed.emit(other_viewer.rewindButton.isChecked())
    ...
    def sync_with(self, other_viewer):
        ...
        self.current_time = other_viewer.current_time
        self.update_plot()  # Update the plot based on the new time
        # Optionally add any additional state synchronization here
    ...
    def pause(self):
        self.plotting_instance.pause()  # Call the pause method in Plotting

    def play(self):
        self.plotting_instance.play()  # Call the play method in Plotting

    def toggle_rewind(self, enabled):
        self.plotting_instance.toggle_rewind(enabled)
    ...
    def update_data(self, data_list):
        ...
        self.data_list.extend(data_list)  # Append new data to existing data
        self.plotting_instance.reset_animation()
